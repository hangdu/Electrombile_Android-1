		...
		if (INSTANCE == null) {
			if (ExceptionHelper.isClassAvailable("java.util.ResourceBundle")) {
				try {
					...
					INSTANCE = (MessageCatalog)Class.forName("org.eclipse.paho.client.phao.mqttv3.internal.ResourceBundleCatalog").newInstance();
				} catch (Exception e) {
					return "";
				}
			} else if (ExceptionHelper.isClassAvailable("org.eclipse.paho.client.phao.mqttv3.internal.MIDPCatalog")){
				try {
					INSTANCE = (MessageCatalog)Class.forName("org.eclipse.paho.client.phao.mqttv3.internal.MIDPCatalog").newInstance();
				} catch (Exception e) {
					return "";
				}
			}
