...
import phao.service.MqttService;
...
public class PushService extends MqttService {
    ...
    private void initConnection() {
        ...
        clientHandler = getClient(MQTT_HOST + ":1883", MQTT_CLIENT_ID, null, null);
        //defaultToken = getPendingDeliveryTokens(clientHandler)[0].getMessage().;
        MqttConnectOptions mco = new MqttConnectOptions();
        mco.setCleanSession(MQTT_CLEAN_START);
        mco.setWill("error", "i am lost...".getBytes(), 0, false);
        try {
            connect(clientHandler, mco, null, defaultToken);
            subscribe();
    ...
    private void subscribe() {
        ...
        int[] qos = {MQTT_QUALITY_OF_SERVICE, MQTT_QUALITY_OF_SERVICE, MQTT_QUALITY_OF_SERVICE};
        subscribe(clientHandler, topic, qos, "", defaultToken);
        LogUtil.log.i("Connection established to " + MQTT_HOST + " on topic " + topic1);
        LogUtil.log.i("Connection established to " + MQTT_HOST + " on topic " + topic2);
        LogUtil.log.i("Connection established to " + MQTT_HOST + " on topic " + topic3);
    }
...
//    private static final int INITIAL_RETRY_INTERVAL = 1000 * 10;
...
//    private long mStartTime;
    ...
    public static String defaultToken = "defaultToken";
...
//
    ...
    public void sendMessage1(byte[] message) {
        ...
        try {
            publish(clientHandler, "app2dev/" + settingManager.getIMEI() + "/cmd",
                    message, 0, false, null, defaultToken);
        } catch (MqttException e) {
            LogUtil.log.i("发送消息发生错误：" + e.toString());
        }
...
//            // message.getBytes(), 0, false);
//            Log.d(TAG, " settingManager.getIMEI():" + settingManager.getIMEI());
//            publish("app2dev/" + settingManager.getIMEI() + "/cmd",
//                    message, 0, false);
//        } catch (MqttException e) {
